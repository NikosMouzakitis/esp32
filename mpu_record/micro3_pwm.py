...
# Load datasets
X1, y1 = load_data('increase.txt', 0)  # Class 0 for LED1
X2, y2 = load_data('decrease.txt', 1)  # Class 1 for LED2
X3, y3 = load_data('normal_pwm.txt', 2)      # Class 2 for Neutral
...
avg_inc = np.mean(X1, axis=0)
avg_dec = np.mean(X2, axis=0)
avg_neutral = np.mean(X3, axis=0)

print("Raw averages before scaling:")
print("INC:", ["%.4f" % x for x in avg_inc])
print("DEC:", ["%.4f" % x for x in avg_dec])
...
scaled_inc = scaler.transform([avg_inc])[0]
scaled_dec = scaler.transform([avg_dec])[0]
scaled_neutral = scaler.transform([avg_neutral])[0]

print("\nReference vectors after scaling (copy to ESP32 code):")
print("const float led1_ref[] = {" + ", ".join(["%.4ff" % x for x in scaled_inc]) + "};")
print("const float led2_ref[] = {" + ", ".join(["%.4ff" % x for x in scaled_dec]) + "};")
...
with open('PWMClassifier.h', 'w') as f:
    f.write(c_code)

print("\nC code generated successfully in PWMClassifier.h")
