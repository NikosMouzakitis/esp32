...
# Calculate and print average samples
avg_led1 = np.mean(X1, axis=0)
avg_led2 = np.mean(X2, axis=0)

print("\nAverage sample for LED1_TOGGLE class:")
print("[" + ", ".join([f"{x:.2f}" for x in avg_led1]) + "]")

print("\nAverage sample for LED2_TOGGLE class:")
print("[" + ", ".join([f"{x:.2f}" for x in avg_led2]) + "]")
